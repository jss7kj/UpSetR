...
Make_main_bar <- function(Main_bar_data, Q, show_num, ratios, customQ, number_angles,
  ...
  Main_bar_plot <- (ggplot(data = Main_bar_data, aes_string(x = "x", y = "freq")) 
                    ...
                    + theme(panel.background = element_rect(fill = "white"),
                            ...
                            axis.title.y = element_text(vjust = -0.8, size = 15.3), axis.text.y = element_text(vjust=0.3,
                                                                                                            size=12 )))
  if((show_num == "yes") || (show_num == "Yes")){
    Main_bar_plot <- (Main_bar_plot + geom_text(aes_string(label = "freq"), size = 6, vjust = -1,
